...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <button onclick="showDetails('${product.image}','${product.title}','${product.price}','${product.rating.rate}','${product.description}') " id="details-btn" class="btn btn-danger">Details</button></div>
      `;
...
// for showing detailes

const showDetails= (image, title, price, rate, description) =>{
  const countryDe = document.getElementById('detailes');
  countryDe.innerHTML = `
  
  <div class="card detaile-product" style="width: 28rem;">
  <img class="card-img-top" src="${image}" alt="Card image cap" style="width: 200px;">
  <div class="card-body">
  <h3>${title}</h3>
  <h2>Price: $ ${price}</h2>
  <p>Rate:${rate}</p>
